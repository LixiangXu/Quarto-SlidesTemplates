...
-- Define a function to generate HTML code for an iframe element specified for html page
local function iframe_helper_html(file_name, height, width, full_screen_link, template, desc)
  -- Check if the file exists
  checkFile(file_name)

  -- Define a template for displaying a full-screen link
  local template_full_screen = [[
    <p><a href=%q target="_blank">%s</a></p>
  ]]
  ...
  local combined_str = string.format(
    [[%s %s]],
    -- Include full-screen link if specified
    (full_screen_link == "true" and string.format(template_full_screen, file_name, desc) or ""),
    -- Insert the iframe template with file name and height
    string.format(template, file_name, height, width)
  )
...
local function html(args, kwargs, meta, raw_args)
  ...
  local height = getOption(kwargs, "height", "300px")
  local width = getOption(kwargs, "width", "600px")
  local full_screen_link = getOption(kwargs, "full-screen-link", "true")
  local desc = getOption(kwargs, "desc", "Link")
  ...
  return iframe_helper_html(file_name, height, width, full_screen_link, template_html, desc)
end
...
local function pdf(args, kwargs, meta)
  ...
  local height = getOption(kwargs, "height", "100%")
